...
void TrackingAction::PreUserTrackingAction(const G4Track* track)
{
  ...
  //
  G4int ih = 9;
  if (particle == G4Electron::Electron()) ih = 4;
  else if (particle == G4Positron::Positron());
  else if (particle == G4NeutrinoE::NeutrinoE()||
           particle == G4AntiNeutrinoE::AntiNeutrinoE());
  else if (particle == G4Gamma::Gamma()) ih = 5;
  else if (particle == G4Alpha::Alpha());
  else if (fCharge > 2.);
  
  // Rishita
  if (ih && ih != 9) G4AnalysisManager::Instance()->FillH1(ih, Ekin);

  //if(fProducts && ID == 1) {fProducts->AddDecayChain(name);}
  //else if(fProducts) {fProducts->AddProductChain("---> " + name);}
