...
#include "G4IsotopeProperty.hh" // Rishita
...
#include "G4Alpha.hh"
#include "G4Proton.hh"
//#include "G4VGammaDeexcitation" // Rishita
...
G4RadioactiveDecay::DoDecay(const G4ParticleDefinition& theParticleDef)
{
  ...
  G4String daughterName; // Rishita
  G4int parentZ = theParticleDef.GetAtomicNumber();
  G4int parentA = theParticleDef.GetAtomicMass();
  
  //G4String parentName = theParticleDef.GetParticleName(); // Rishita

  G4ParticleDefinition* firstP;
  G4ParticleDefinition* secondP;
  G4DynamicParticle* aPop;
  G4DynamicParticle* bPop;
  for (G4int pindex=products->entries()-1; pindex >=0 ; pindex--) {
      aPop = products->operator[](pindex);

	// Rishita -- assuming that the order of products is in the same
	// order as the secondaries vector for the track


      daughterZ = aPop->GetDefinition()->GetAtomicNumber();
      daughterA = aPop->GetDefinition()->GetAtomicMass();
    //  daughterName = aPop->GetDefinition()->GetParticleName(); // Rishita
      if(daughterA>0) break;
  }

  for (G4int pindex = 0; pindex < products->entries()-1; pindex++) {
	if(products->entries() == 1) { break; }

	aPop = products->operator[](pindex);
	bPop = products->operator[](pindex+1);

	firstP = aPop->GetDefinition();
	secondP = bPop->GetDefinition();
      
      if ((firstP == G4Gamma::Definition() ||
	   firstP == G4Electron::Definition()) && 
	  (secondP == G4Gamma::Definition() ||
	   secondP == G4Electron::Definition())) {
	break;
      }

      if (firstP->GetAtomicMass() > 0 || secondP->GetAtomicMass() > 0) {
	break;
      }
  }
...
/*	if(firstP) { G4cout << "First Particle: " << firstP->GetParticleName() << G4endl; }
	else {G4cout << "firstP is NULL" << G4endl; }

	if(secondP) { G4cout << "Second Particle: " << secondP->GetParticleName() << G4endl; }
	else { G4cout << "secondP is NULL" << G4endl; }
*/
	if(firstP == G4Gamma::Definition()) {

		if(secondP == G4Gamma::Definition()) {
			G4NuclearLevelStore::GetInstance()->SetCaseNumber(0); 
			// G4cout << "case zero" << G4endl;
			// G4NuclearLevelStore::GetInstance()->SetGammaGamma(true);

		} else if (secondP == G4Electron::Definition()) {
			G4NuclearLevelStore::GetInstance()->SetCaseNumber(1); 
			// G4cout << "case one" << G4endl;
			// G4NuclearLevelStore::GetInstance()->SetGammaElectron(true);

		} else { G4NuclearLevelStore::GetInstance()->SetCaseNumber(-40);  
			 // G4cout << "case forty" << G4endl; 
		       }

	} else if (firstP == G4Electron::Definition()) {

		if (secondP == G4Gamma::Definition()) {
			G4NuclearLevelStore::GetInstance()->SetCaseNumber(2); 
			// G4cout << "case two" << G4endl;
			// G4NuclearLevelStore::GetInstance()->SetElectronGamma(true);

		} else if (secondP == G4Electron::Definition()) {
			G4NuclearLevelStore::GetInstance()->SetCaseNumber(3); 
			// G4cout << "case three" << G4endl;
			// G4NuclearLevelStore::GetInstance()->SetElectronElectron(true);

		} else { G4NuclearLevelStore::GetInstance()->SetCaseNumber(-41); 
			 // G4cout << "case forty-one" << G4endl; 
			 }
	} else {
//		G4cout << "else reached!" << G4endl; 
		G4NuclearLevelStore::GetInstance()->SetCaseNumber(-42); 
		 // G4cout << "case forty-two" << G4endl; 
	}

	G4int caseno = G4NuclearLevelStore::GetInstance()->GetCaseNumber();	
//	G4cout << "case end G4RadioactiveDecay: " << caseno << G4endl;
  ...
  //
  // Rishita: NOTE -- This occurs regardless of the particle type. The correlation will be isotropic 
  // if not gamma or electron
