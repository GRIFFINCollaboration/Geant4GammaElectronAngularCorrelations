...
G4bool G4DiscreteGammaDeexcitation::CanDoTransition(G4Fragment* nucleus) 
{
  ...
  if(excitation <= _tolerance) {
	...
	// Rishita
    G4cout << "excitation <= _tolerance" << G4endl;
    return false;
  }
  ...
  if(!levelManager ||
     excitation > levelManager->MaxLevelEnergy() + _tolerance) { 
	G4bool Rishita = excitation > levelManager->MaxLevelEnergy() + _tolerance;
	G4cout << "!levelManager: " << !levelManager << " OR excitation > levelManager->MaxLevelEnergy() + _tolerance: " << Rishita << G4endl;
	return false; }
  ...
  // long lived level	
  if (!level || level->HalfLife() > maxhl) { 
	// Rishita
	G4bool Rishita2 = level->HalfLife() > maxhl;
	G4cout << "!level: " << !level << " OR level->HalfLife() > maxhl: " << Rishita2 << G4endl;
	return false; }
