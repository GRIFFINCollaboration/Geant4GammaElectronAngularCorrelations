...
//#include "G4DiscreteGammaTransition.hh" // Rishita
...
// Rishita --------------------------------------------------------------------------------------------
	ReadDataItem(dataFile, _b1) &&
	ReadDataItem(dataFile, _b2) &&
	ReadDataItem(dataFile, _b3) &&
	ReadDataItem(dataFile, _b4) &&
	ReadDataItem(dataFile, _b5) &&
	ReadDataItem(dataFile, _b6) &&
	ReadDataItem(dataFile, _alphL1) &&
	ReadDataItem(dataFile, _alphL2) &&
	ReadDataItem(dataFile, _electrue));
}
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
...
{
  G4double  ji, jo, jf, L1, L1p, L2, L2p, delta1, delta2, b_a1, b_a2, b_a3, b_a4, b_a5, b_a6;
  G4double  b_b1, b_b2, b_b3, b_b4, b_b5, b_b6,  alph_aL, alph_aLp, alph_bL, alph_bLp;
  G4bool    electrue_a, electrue_b;
...
/*	G4cout << " -------------------------- READING START -------------------------------------" << G4endl;
	G4cout << "\n" << G4endl;
	G4cout << "_levelEnergyMultipole: " << _levelEnergyMultipole << G4endl;
	G4cout << "thisLevel->Energy(): " << thisLevel->Energy() << G4endl;
	G4cout << "_gammaEnergyMultipole: " << _gammaEnergyMultipole << G4endl;
	G4cout << "thisLevel->GammaEnergies()[i]: " << thisLevel->GammaEnergies()[i] << G4endl; 
	G4cout << "\n" << G4endl;
	G4cout << "_L1: " << _L1 << G4endl;
	G4cout << "_L2: " << _L2 << G4endl;
	G4cout << "\n" << G4endl;
	G4bool firstcheck = _levelEnergyMultipole == thisLevel->Energy();
*/	G4bool secondcheck = (std::abs(_gammaEnergyMultipole - thisLevel->GammaEnergies()[i]) < 0.000001); 
/*	G4cout << "firstcheck: " << firstcheck << " secondcheck: " << secondcheck << G4endl;*/
      if((_levelEnergyMultipole == thisLevel->Energy()) && secondcheck ) {
        thisLevel->FillL1(i, _L1);
        thisLevel->FillL2(i, _L2);
        thisLevel->FillMixingRatio(i, _mixingRatio);
        thisLevel->Fillb1(i, _b1);
        thisLevel->Fillb2(i, _b2);
        thisLevel->Fillb3(i, _b3);
        thisLevel->Fillb4(i, _b4);
        thisLevel->Fillb5(i, _b5);
        thisLevel->Fillb6(i, _b6);
        thisLevel->FillalphL1(i, _alphL1);
        thisLevel->FillalphL2(i, _alphL2);
        thisLevel->Fillelectrue(i, _electrue);
	//	G4cout << "Fill i: " << i << G4endl;
	//	G4cout << "Fill -- thisLevel->L1()[i]: " << thisLevel->L1()[i] << G4endl;
	//	G4cout << "\n" << G4endl;
	//	G4cout << "--------------------------- READING END ---------------------------------------" << G4endl;
      }
    }
  }
  inFile.close();
  ...
  for (G4int allLevels = 0; allLevels < nLevels; allLevels++) { // loop over all possible levels
	...
	//G4cout << "thisLevel i: " << i << G4endl;
      gammaEnergy   = thisLevel->GammaEnergies()[i];
      L2            = thisLevel->L1()[i];
	//G4cout << "thisLevel->L1()[i]: " << thisLevel->L1()[i] << G4endl;
      L2p           = thisLevel->L2()[i];
...
// Rishita --------------------------------------------------
      ...
      b_b5		= thisLevel->B5()[i];
      b_b6		= thisLevel->B6()[i];
      alph_bL		= thisLevel->AlphL1()[i];
      alph_bLp		= thisLevel->AlphL2()[i];
      electrue_b	= thisLevel->Electrue()[i];
// ----------------------------------------------------------
//if (electrue_b != 0) {G4cout << "electrue_b in Manager: " << electrue_b << G4endl;}	
	...
	// Rishita -------------------------------------------------------------
      		b_a1		= higherLevel->B1()[i];
      		b_a2		= higherLevel->B2()[i];
      		b_a3		= higherLevel->B3()[i];
      		b_a4		= higherLevel->B4()[i];
      		b_a5		= higherLevel->B5()[i];
      		b_a6		= higherLevel->B6()[i];
      		alph_aL		= higherLevel->AlphL1()[i];
      		alph_aLp	= higherLevel->AlphL2()[i];
      		electrue_a	= higherLevel->Electrue()[i];
		//if (electrue_a != 0) {G4cout << "electrue_a in Manager: " << electrue_a << G4endl;}
        // ----------------------------------------------------------------------
            }
	...
	// Rishita -------------------------------------------------------------
      		...
      		alph_aLp	= higherLevel->AlphL2()[i];
      		electrue_a	= higherLevel->Electrue()[i];
		//if (electrue_a != 0) {G4cout << "electrue_a in Manager: " << electrue_a << " CASE 2" << G4endl;}
        // ----------------------------------------------------------------------
              }
            }
	...
	/*	G4cout << "-----------------------------------------------------" << G4endl;
		G4cout << "levelEnergy: " << levelEnergy*1000 << " keV" << G4endl;
		G4cout << "higherLevelEnergy: " << higherLevelEnergy*1000 << " keV" << G4endl;
		G4cout << "L1: " << L1 << " L1p: " << L1p << G4endl;
		G4cout << "L2: " << L2 << " L2p: " << L2p << G4endl;
		G4cout << "\n" << G4endl;*/
		}

		//G4NuclearLevelStore::GetInstance()->StoreVars(_alphL1, elecfirst);
// Rishita -- if you include DiscreteGammaTransition, you can put a case here passing different values of b1->b6s
            thisLevel->GenerateWThetaParameters(i, indexHigher, higherLevelEnergy, lowerGammaEnergy, higherGammaEnergy, ji, jo, jf, L1, L1p, L2, L2p, delta1, delta2, boolGoodLevelToOutputToScreen, b_a1, b_a2, b_a3, b_a4, b_a5, b_a6, alph_aL, alph_aLp, electrue_a, b_b1, b_b2, b_b3, b_b4, b_b5, b_b6, alph_bL, alph_bLp, electrue_b);
            indexHigher++;
          }
        }
