...
G4bool elecfirst = false;
G4bool* elecfirstp = &elecfirst; // can put this in store
                   ...
                   G4Fragment* nucleus)
{
  ...
  // Rishita
//  G4cout << "DoChain" << G4endl;
  if(CanDoTransition(nucleus)) {
    for(size_t i=0; i<100; ++i) {
	
	_transition->SetEnergyFrom(nucleus->GetExcitationEnergy());
	
	if (products->size() == 0) { G4NuclearLevelStore::GetInstance()->SetFirstProduct(true); }
	else {  G4NuclearLevelStore::GetInstance()->SetFirstProduct(false); }
...
// Rishita TODO: SELECT GAMMA AFFECT ELECTRON?
  ...
  // 3 == electron-electron
	...
	//	G4cout << "first decay" <<G4endl; 
		...
		caseno = -5;
	}  // Rishita -> add back bond energy? remove the subtraction from Transition file?

	else {
		caseno = G4NuclearLevelStore::GetInstance()->GetCaseNumber();
		if (caseno == 0 || caseno == 2) { // last particle was a gamma
		...
		} else if (caseno == 1 || caseno == 3) { // last particle was an electron
		...
		} else if (caseno == -5) { // this is the second particle generated
  ...
  G4bool firstdecay = G4NuclearLevelStore::GetInstance()->GetFirstGammaDecay();

  //if (dtransition) {
  	G4double levelEnergy      = dtransition->GetLevelEnergy();
  //}

	// Rishita: removed dtransition->IsAGamma() check -- 
	// 	IC is now taken care of and transitions can only be electrons or gammas (should fix later)
 ...
 /* if(dtransition && G4NuclearLevelStore::GetInstance()->GetUserFilesMultipole(aNucleus->GetZ(), aNucleus->GetA()) && caseno >= 0) { // if we have given the multipole file
	...
	// TODO? if first decay, save theta -- then remove the update first decay above and put here
