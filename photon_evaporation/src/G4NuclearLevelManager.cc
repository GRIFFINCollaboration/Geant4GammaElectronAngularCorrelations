...
// Rishita Gudapati- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
      ...
      ReadDataItem(dataFile, _alphL) &&
      ReadDataItem(dataFile, _alphLp));
}
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
...
void G4NuclearLevelManager::AddMulipole(G4int Z, G4int A, G4String myfilename)
{
  ...
  G4double  bf1, bf2, bf3, bf4, bf5, bf6, alphL1, alphL1p;
  G4double  bs1, bs2, bs3, bs4, bs5, bs6, alphL2, alphL2p;
  ...
  while (ReadMultipole(inFile)) {
	...
	thisLevel->FillAlphL(i, _alphL);
	thisLevel->FillAlphLp(i, _alphLp);
      }
    }
  }
  inFile.close();
...
// Rishita Gudapati ----------------------------------------------------
      ...
      bs5 = thisLevel->B5()[i];
      bs6 = thisLevel->B6()[i];
      alphL2 = thisLevel->AlphL()[i];
      alphL2p = thisLevel->AlphLp()[i];	
// ---------------------------------------------------------------------
