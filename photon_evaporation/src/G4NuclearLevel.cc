...
// b1 to b3 belong to a2
// b4 to b6 belong to a4
...
const std::vector<G4double>& G4NuclearLevel::AlphL() const
{
  return _alphL;
}

const std::vector<G4double>& G4NuclearLevel::AlphLp() const
{
  return _alphLp;
}
...
G4double alphL1p, G4double bs1, G4double bs2, G4double bs3, G4double bs4, G4double bs5, G4double bs6, G4double alphL2, G4double alphL2p) {
  ...
  G4double delta_ex1 = std::pow((alphL1p / alphL1), 0.5) * delta1;
  G4double delta_ex2 = std::pow((alphL2p / alphL2), 0.5) * delta2;
  ...
  // gamma-electron coefficients
  a2_ge  = firstTransition(2,Jo,Ji,L1,L1p,delta1,1,1,1)  * secondTransition(2,Jf,Jo,L2,L2p,delta_ex2,bs1,bs2,bs3);
  a4_ge  = firstTransition(4,Jo,Ji,L1,L1p,delta1,1,1,1)  * secondTransition(4,Jf,Jo,L2,L2p,delta_ex2,bs4,bs5,bs6);
  ...
  // electron-gamma coefficients
  a2_eg  = firstTransition(2,Jo,Ji,L1,L1p,delta_ex1,bf1,bf2,bf3)  * secondTransition(2,Jf,Jo,L2,L2p,delta2,1,1,1);
  a4_eg  = firstTransition(4,Jo,Ji,L1,L1p,delta_ex1,bf4,bf5,bf6)  * secondTransition(4,Jf,Jo,L2,L2p,delta2,1,1,1);
  ...
  // electron-electron coefficients
  a2_ee  = firstTransition(2,Jo,Ji,L1,L1p,delta_ex1,bf1,bf2,bf3)  * secondTransition(2,Jf,Jo,L2,L2p,delta_ex2,bs1,bs2,bs3);
  a4_ee  = firstTransition(4,Jo,Ji,L1,L1p,delta_ex1,bf4,bf5,bf6)  * secondTransition(4,Jf,Jo,L2,L2p,delta_ex2,bs4,bs5,bs6);
  ...
  if(boolGoodLevelToOutputToScreen && (a2_gg != 0 || a4_gg != 0)) {
    ...
    G4cout << "delta_ex1 = " << delta_ex1 << " - " << "delta_ex2 = " << delta_ex2 << G4endl;
    G4cout << "\n" << G4endl;
    }
  ...
  for(G4int i = 0; i < 180000 ; i++ ) {
    ...
    wTheta_gg = (1 + a2_gg * P2 
	           + a4_gg * P4 
		   + a6_gg * P6 
		   + a8_gg * P8 
		   + a10_gg * P10) * std::sin(thetaRad);
    if (wTheta_gg > thisMaxWTheta_gg) { thisMaxWTheta_gg = wTheta_gg; }

    wTheta_ge = (1 + a2_ge * P2
		   + a4_ge * P4 
		   + a6_ge * P6 
		   + a8_ge * P8 
  		   + a10_ge * P10) * std::sin(thetaRad);
    if (wTheta_ge > thisMaxWTheta_ge) { thisMaxWTheta_ge = wTheta_ge; }

    wTheta_eg = (1 + a2_eg * P2 
		   + a4_eg * P4 
		   + a6_eg * P6 
		   + a8_eg * P8
		   + a10_eg * P10) * std::sin(thetaRad);
    if (wTheta_eg > thisMaxWTheta_eg) { thisMaxWTheta_eg = wTheta_eg; }

    wTheta_ee = (1 + a2_ee * P2
		   + a4_ee * P4 
		   + a6_ee * P6 
		   + a8_ee * P8
		   + a10_ee * P10) * std::sin(thetaRad);
...
// Rishita Gudapati -- modified to include b parameters 
